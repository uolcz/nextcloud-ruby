RSpec.describe Nextcloud::Ruby::Configuration do
  ...
  context 'with configuration block' do
    it 'returns the correct dav_url' do
      expect(Nextcloud::Ruby.configuration.dav_url).to eq(URI('https://example.com'))
    end
    ...
    it 'returns the correct dorectories' do
      expect(Nextcloud::Ruby.configuration.directories).to match_array(['a/b/c', 'd/e/f'])
    end
  end
  ...
  context 'without configuration block' do
    ...
    it 'raises a configuration error for dav_url' do
      expect { Nextcloud::Ruby.configuration.dav_url }.to raise_error(Nextcloud::Ruby::Errors::ConfigNotSet)
    end

    it 'raises a configuration error for username' do
      expect { Nextcloud::Ruby.configuration.username }.to raise_error(Nextcloud::Ruby::Errors::ConfigNotSet)
    end

    it 'raises a configuration error for password' do
      expect { Nextcloud::Ruby.configuration.password }.to raise_error(Nextcloud::Ruby::Errors::ConfigNotSet)
    end

    it 'raises a configuration error for directories' do
      expect { Nextcloud::Ruby.configuration.directories }.to raise_error(Nextcloud::Ruby::Errors::ConfigNotSet)
    end
  end
